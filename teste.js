...
let ipads = productDatabase.ipads;
let watch = productDatabase["apple-watch"];
let acessorios = productDatabase.acessorios;

const listIpads = ipads.map((product) => {
...
const listWatch = watch.map((product) => {
  const { id, image, ...rest } = product;
...
const listAcessórios = acessorios.map((product) => {
  const { id, image, ...rest } = product;
