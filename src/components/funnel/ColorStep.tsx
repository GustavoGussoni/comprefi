...
const ColorStep: React.FC<ColorStepProps> = ({ 
  ...
  const loadColors = async () => {
    try {
      ...
      // Fallback colors
      setAvailableColors(['Preto', 'Branco', 'Azul', 'Vermelho', 'Rosa', 'Amarelo']);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="text-gray-400">
          Selecione a cor exata do seu <span className="text-blue-400 font-medium">{modelName}</span>
        </p>
      </div>
        ...
        <div className="mt-6 p-4 bg-green-900 bg-opacity-30 border border-green-700 rounded-lg">
          <div className="flex items-center">
            ...
            <p className="text-green-300">
              Cor <span className="font-medium">{selectedColor}</span> selecionada!
            </p>
          </div>
        </div>
      )}
        ...
        <div className="flex items-start">
          ...
          <div>
            <p className="text-blue-300 text-sm">
              <strong>Dica:</strong> A cor exata pode influenciar no valor de revenda. 
              Cores mais populares como Preto e Branco geralmente têm melhor aceitação no mercado.
            </p>
          </div>
        </div>
      </div>
